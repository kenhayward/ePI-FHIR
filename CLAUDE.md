...
**Traceability is mechanical, not manual.** [design/traceability/](design/traceability/) holds a
V-model chain: requirement -> design function -> unit test, and requirement -> integration test.
The four matrices are generated by `tools/build-traceability.py` and verified in CI - never edit
them by hand.

The half you must not break: **unit and integration tests are discovered from their own names**,
so naming is load-bearing. Name a unit test for the design function it verifies
(`FN_CC_006_serialises_without_content_loss`), an integration test for its scenario
(`IT_001_bundle_round_trips_through_create_and_read`), or either for the requirement
(`CAP_SCM_010_round_trip_is_lossless`). A test naming a function or scenario that is not in the
registry fails the build, as does a registry entry pointing at a requirement that does not exist.
