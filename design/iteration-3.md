...
| PDF determinism is proven against a stand-in that reproduces the engine's measured behaviour, not against the engine itself. A container-backed project written for it crashed the test host on start-up and was removed rather than shipped: a test project that cannot run looks exactly like one that passes, which is the defect the orphaned-project CI guard exists for | PR 58 | **Before rendering is demonstrated** - IT-019 |
